...
// ==== Абсолютные ссылки на картинки (GitHub Pages) ====
const MEDIA = (file) => `https://yoimdely.github.io/prostory-kryma2/media/${file}`;

// ==== SEO + ШРИФТЫ ====
function injectSEO() {
  if (typeof document === "undefined") return;

  const OG_IMG = MEDIA("51.jpeg"); // hero для превью
...
export default function App() {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#FFF8F2", color: "#1F1B16", fontFamily: "Montserrat, sans-serif" }}>
      {/* ДЕКОР — волнистый фон шапки */}
      <div aria-hidden className="pointer-events-none fixed inset-0 -z-10">
        <svg viewBox="0 0 1440 600" className="w-full h-[520px]" preserveAspectRatio="none">
          <defs>
            <linearGradient id="g1" x1="0" x2="1" y1="0" y2="1">
              <stop offset="0%" stopColor="#F6E6D9"/>
              <stop offset="100%" stopColor="#FFF8F2"/>
            </linearGradient>
          </defs>
          <path d="M0,120 C240,240 360,0 600,80 C840,160 960,60 1200,120 C1320,150 1380,170 1440,160 L1440,0 L0,0 Z" fill="url(#g1)" opacity="0.9"/>
          <path d="M0,200 C220,260 380,120 620,160 C860,200 1000,120 1240,180 C1340,205 1390,230 1440,220 L1440,0 L0,0 Z" fill="#FBEFE6" opacity="0.8"/>
        </svg>
      </div>
      ...
      <header className="sticky top-0 z-30 border-b backdrop-blur"
        ...
        <div className="max-w-6xl mx-auto px-5 py-3 flex items-center gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-6 text-[13px] mx-auto" aria-label="Главное меню">
            {[["О проекте","#about"],["Локация","#location"],["Планировки","#plans"],["Галерея","#gallery"],["Контакты","#buy"]]
              .map(([t,href]) => <a key={href} href={href} className="hover:text-orange-600 transition-colors" style={{ color: "#4B3B30" }}>{t}</a>)}
          </nav>

          <div className="ml-auto hidden sm:flex items-center gap-3">
            <a href="#cta" className="px-4 py-2 rounded-2xl hover:shadow-md" style={{ backgroundColor: "#C65D3A", color: "#FFF8F2" }}>
              Подбор квартиры
            </a>
      ...
      <section className="relative overflow-hidden">
        <div className="relative max-w-6xl mx-auto px-4 py-16 md:py-24 grid md:grid-cols-2 gap-10">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.5}}>
            <h1 className="text-[34px] md:text-[48px] lg:text-[56px] leading-[1.1] font-extrabold"
                style={{fontFamily:'Prata, serif', color:'#2B2118', wordBreak:'keep-all'}}>
          ...
          <motion.div className="rounded-3xl overflow-hidden shadow-lg border"
            style={{height:520, borderColor:'#EAD6C4'}} initial={{opacity:0, scale:0.98}} animate={{opacity:1, scale:1}} transition={{duration:0.6}}>
            <img
              src={MEDIA("51.jpeg")}
        ...
        {/* нижняя волна секции */}
        <svg viewBox="0 0 1440 140" className="block w-full h-[70px]" preserveAspectRatio="none" aria-hidden>
          <path d="M0,40 C180,90 360,0 540,40 C720,80 900,20 1080,60 C1260,100 1350,100 1440,80 L1440,140 L0,140 Z" fill="#FFF3EA"/>
        </svg>
      </section>
          ...
          <div className="p-6 rounded-2xl border shadow" style={{backgroundColor:'#FFFFFF', borderColor:'#EAD6C4'}}>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mt-3 text-sm" style={{color:'#4B3B30'}}>
              {["Энергоэффективность и комфорт","Ликвидность для аренды/перепродажи","Удобные планировки","Паркинг и коммерция на территории"]
                .map((t,i)=>(<div key={i} className="p-4 rounded-xl border" style={{backgroundColor:'#FFF8F2', borderColor:'#EAD6C4'}}>{t}</div>))}
            </div>
          </div>
        </div>
      </section>
